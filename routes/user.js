// ROUTES file: user.js
// fetch requests from UserAPI (frontend) come here
...
// PUT - add favouriteListing --------------------------------------
router.put('/favouriteListing/', Utils.authenticateToken, (req, res) => {  
  // validate check
  if(!req.body.listingId){
    return res.status(400).json({
      message: "No listing specified"
    })
  }
  // add listingId to favouriteListing field (User model) (array - push)
  // push = adds an extra item to an array
  User.updateOne({
    _id: req.user._id
  }, {
    $push: {
      favouriteListings: req.body.listingId
    }
  })
    .then((user) => {            
      res.json({
    ...
    .catch(err => {
      console.log(err)
      res.status(500).json({
        message: "Problem adding favourite listing"
      })
    })
})
...
router.get('/:id', Utils.authenticateToken, (req, res) => {
  ...
  User.findById(req.params.id).populate('favouriteListings')
    .then(user => {
      res.json(user)
    })
    .catch(err => {
